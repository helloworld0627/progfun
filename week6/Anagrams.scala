...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    // expand the pair to pairs which cnt to zero
    def expandPair(p: (Char, Int)): List[(Char, Int)] = {
      if (p._2 < 0) List()
      else (p._1, p._2) :: expandPair((p._1, p._2 - 1))
    }

    def combine(occurrencesLst: List[Occurrences]): List[Occurrences] = occurrencesLst match {
      case List() => List(List())
      case sameCharPairs :: rest => for (pair <- sameCharPairs; combined <- combine(rest))
        yield if (pair._2 > 0) pair :: combined else combined
    }
    val pairLst = occurrences.map { p => expandPair(p) }
    combine(pairLst)
  }
